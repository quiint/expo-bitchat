...
class BluetoothMeshService(private val context: Context) {
    ...
    fun sendEncryptedChannelMessage(content: String, mentions: List<String>, channel: String, channelKey: javax.crypto.spec.SecretKeySpec) {
        if (content.isEmpty() || channel.isEmpty()) return
        ...
        serviceScope.launch {
            ...
            message.toBinaryPayload()?.let { messageData ->
                try {
                    val encryptedPayload = encryptionService.encryptWithSymmetricKey(messageData, channelKey)
                    ...
                    Log.e(TAG, "Failed to send encrypted channel message: ${e.message}")
                }
            }
        }
    }
